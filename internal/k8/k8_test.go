...
var _ = Describe("Kubernetes - Unit", Label("unit"), func() {
	...
	Context("when GetAllImages is called with a K8s client", func() {
		...
		It("when GetAllImages is called when same image exists in same namespace but with different version", func() {
			...
			// Add the Pod to the fake dynamic client
			//_, err = fakeDynamicClient.Resource(gvrs["pods"]).Namespace("default").Create(context.TODO(), unstructuredPod, v1.CreateOptions{})
			//if err != nil {
			//	fmt.Println("Error creating pod in fake client:", err)
			//	return
			//}
